...
GST_START_TEST (test_pipeline_delete_successful)
{
  ...
  const gchar *expected = "pipeline_delete pipe";

  ret = gstd_pipeline_delete (manager, pipeline_name);
...
GST_START_TEST (test_pipeline_delete_null_name)
{
  GstdStatus ret;
...
GST_START_TEST (test_pipeline_delete_null_manager)
{
  GstdStatus ret;
  const gchar *pipeline_name = "pipe";

  ret = gstd_pipeline_delete (NULL, pipeline_name);
  assert_equals_int (GSTD_LIB_NULL_ARGUMENT, ret);
}
...
static Suite *
gstd_pipeline_delete_suite (void)
{
  Suite *suite = suite_create ("gstd_pipeline_delete");
  ...
  tcase_add_test (tc, test_pipeline_delete_null_name);
  tcase_add_test (tc, test_pipeline_delete_null_manager);

  return suite;
}

GST_CHECK_MAIN (gstd_pipeline_delete);
