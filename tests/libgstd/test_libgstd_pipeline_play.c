...
GST_START_TEST (test_pipeline_play_successful)
{
  GstdStatus ret;
  const gchar *pipeline_name = "pipe";
  const gchar *expected = "pipeline_play pipe";

  ret = gstd_pipeline_play (manager, pipeline_name);
...
GST_START_TEST (test_pipeline_play_null_name)
{
  GstdStatus ret;
...
GST_START_TEST (test_pipeline_play_null_manager)
{
  GstdStatus ret;
  const gchar *pipeline_name = "pipe";

  ret = gstd_pipeline_play (NULL, pipeline_name);
  assert_equals_int (GSTD_LIB_NULL_ARGUMENT, ret);
}
...
static Suite *
gstd_pipeline_play_suite (void)
{
  Suite *suite = suite_create ("gstd_pipeline_play");
  TCase *tc = tcase_create ("general");

  suite_add_tcase (suite, tc);

  tcase_add_checked_fixture (tc, setup, teardown);
  tcase_add_test (tc, test_pipeline_play_successful);
  tcase_add_test (tc, test_pipeline_play_null_name);
  tcase_add_test (tc, test_pipeline_play_null_manager);

  return suite;
}

GST_CHECK_MAIN (gstd_pipeline_play);
