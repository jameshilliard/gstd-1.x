...
GST_START_TEST (test_pipeline_list_signals_success)
{
  ...
  const gchar *expected = "list_signals pipe element";
  gchar **response = NULL;
  int array_lenght;

  ret =
      gstd_pipeline_list_signals (manager, pipeline_name, element_name,
...
GST_START_TEST (test_pipeline_list_signals_null_pipeline_name)
{
  GstdStatus ret;
  const gchar *element_name = "element";
  gchar **response = NULL;
  ...
  ret =
      gstd_pipeline_list_signals (manager, NULL, element_name, &response,
      &array_lenght);
...
GST_START_TEST (test_pipeline_list_signals_null_element_name)
{
  GstdStatus ret;
  const gchar *pipeline_name = "pipe";
  gchar **response = NULL;
  ...
  ret =
      gstd_pipeline_list_signals (manager, pipeline_name, NULL, &response,
      &array_lenght);
  assert_equals_int (GSTD_LIB_NULL_ARGUMENT, ret);
...
GST_START_TEST (test_pipeline_list_signals_null_response)
{
  ...
  ret =
      gstd_pipeline_list_signals (manager, pipeline_name, element_name, NULL,
      &array_lenght);
  assert_equals_int (GSTD_LIB_NULL_ARGUMENT, ret);
...
GST_START_TEST (test_pipeline_list_signals_null_manager)
{
  GstdStatus ret;
  const gchar *pipeline_name = "pipe";
  const gchar *element_name = "element";
  ...
  ret =
      gstd_pipeline_list_signals (NULL, pipeline_name, element_name,
      &response, &array_lenght);
  assert_equals_int (GSTD_LIB_NULL_ARGUMENT, ret);
...
static Suite *
gstd_pipeline_list_signals_suite (void)
{
  Suite *suite = suite_create ("gstd_pipeline_list_signals");
  ...
  tcase_add_test (tc, test_pipeline_list_signals_null_pipeline_name);
  tcase_add_test (tc, test_pipeline_list_signals_null_element_name);
  tcase_add_test (tc, test_pipeline_list_signals_null_list_lenght);
  tcase_add_test (tc, test_pipeline_list_signals_null_response);
  tcase_add_test (tc, test_pipeline_list_signals_null_manager);

  return suite;
}

GST_CHECK_MAIN (gstd_pipeline_list_signals);
