...
GST_START_TEST (test_pipeline_flush_stop_success_reset)
{
  ...
  const int reset = 1;
  const gchar *expected = "event_flush_start pipe true";

  ret = gstd_pipeline_flush_stop (manager, pipeline_name, reset);
...
GST_START_TEST (test_pipeline_flush_stop_success_no_reset)
{
  ...
  const gchar *pipeline_name = "pipe";
  const int reset = 0;
  const gchar *expected = "event_flush_start pipe false";
...
GST_START_TEST (test_pipeline_flush_stop_null_manager)
{
  GstdStatus ret;
  const int reset = 1;
  const gchar *pipeline_name = "pipe";

  ret = gstd_pipeline_flush_stop (NULL, pipeline_name, reset);
  assert_equals_int (GSTD_LIB_NULL_ARGUMENT, ret);
}
...
static Suite *
gstd_pipeline_flush_stop_suite (void)
{
  Suite *suite = suite_create ("gstd_pipeline_flush_stop");
  ...
  tcase_add_test (tc, test_pipeline_flush_stop_null_manager);

  return suite;
}

GST_CHECK_MAIN (gstd_pipeline_flush_stop);
